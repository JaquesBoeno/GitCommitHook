...
func confirmUpdate(msg tea.Msg, m Model) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "n", "N":
			...
			m.pastResponses = fmt.Sprint(m.pastResponses, fmt.Sprintf("\033[1m%s\033[0m (y/N) %s\n", m.currentQuestion.Label, m.confirmResponse))
...
func confirmView(m Model) string {
	str := strings.Builder{}
	str.WriteString(fmt.Sprintf("%s (y/N) %s", m.currentQuestion.Label, m.confirmResponse))
